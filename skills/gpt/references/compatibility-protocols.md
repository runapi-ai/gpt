# GPT compatibility protocols

Load this reference only for an existing Anthropic Messages or Gemini contents client. Embedding models remain on `/v1/embeddings` and do not use these compatibility protocols.

For Messages, use base URL `https://runapi.ai`, a RunAPI key, a supported exact
generation model ID, `max_tokens`, and messages. Verify final content,
`stop_reason`, and `usage`; streaming ends at `message_stop`.
